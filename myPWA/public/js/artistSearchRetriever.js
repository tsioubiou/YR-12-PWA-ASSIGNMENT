...
Object.keys(searchResults).forEach((key) => { // For every result in there.
    ...
    // Change the h2's writing to include all the info (one thing from every column) on this single result.
    h2.innerHTML = (parseInt(key) + 1).toString() + ". " + (searchResults[key].ArtistName).toString();
